...
class _LibraryScreenState extends State<LibraryScreen> {
  ...
  Future<void> _openModule(ModuleModel module) async {
    setState(() { _selectedModule = module; _loadingFiles = true; _moduleFiles = []; });
    try {
      final files = await _supabase
          ...
          .order('sort_order');
      if (mounted) setState(() { _moduleFiles = List<Map<String, dynamic>>.from(files as List); _loadingFiles = false; });
    } catch (e) {
                      ...
                      ? ListView(children: [
                          ...
                          Center(
                            child: Column(children: [
                              const Icon(Icons.menu_book_outlined, size: 48, color: AppColors.textLight),
                              const SizedBox(height: 12),
              ...
              if (progress < 100)
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      final newPct = progress == 0 ? 10 : (progress + 10).clamp(0, 90);
                      _updateProgress(module.id, newPct);
                    },
                    icon: Icon(progress == 0 ? Icons.play_arrow : Icons.refresh, size: 20),
                    label: Text(progress == 0 ? 'Start Module' : 'Continue Learning',
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (_loadingFiles)
                      ...
                      ..._moduleFiles.map((f) => _FileTile(file: f)),
                  ],
                ),
              ),
            ],
...
class _FileTile extends StatelessWidget {
  ...
  IconData get _icon {
    ...
    if (type.contains('video') || type.contains('mp4')) return Icons.play_circle_outline;
    if (type.contains('image') || type.contains('png') || type.contains('jpg')) return Icons.image_outlined;
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              file['file_name'] ?? file['title'] ?? 'File',
              style: GoogleFonts.nunito(
                  fontSize: 13,
